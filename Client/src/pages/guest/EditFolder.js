...
import { GetProjectForAddFolder, GetProjectForUpdate } from "../../services/Guest/ProjectService";
import { UpdateFolder, GetDetailFolder, DeleteFolder } from "../../services/Guest/FolderService";
        ...
        if (response.success) {
          ...
          if (folderId) {
            ...
            if (folderResponse.success) {
              const { folderName, colorCode, projects } = folderResponse.data;
              setName(folderName);
              setColor(colorCode);
              setProjectSelected(projects.map((project) => project.id));
            } else {
  ...
  const handleDone = async () => {
    if (name) {
      ...
      const response = folderId
        ? await UpdateFolder(folderId, name, color, transformedList, null)
        : await CreateFolder(id, name, color, transformedList, null);
