...
const UpdateSoundDone = () => {
  ...
  useEffect(() => {
    const fetchFileDetails = async () => {
      try {
        const response = await GetDetailSystemFile("SOUNDDONE", id);
        if (response.success) {
          setFormData({
  ...
  return (
    <div className="container mx-auto p-4 max-w-xl bg-white rounded-md shadow-md">
      ...
      <h2 className="text-2xl font-bold mb-4 text-center">Update Sound Done</h2>
      {severity && (
...
export default UpdateSoundDone;
