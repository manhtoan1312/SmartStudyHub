...
import { CreateOrUpdateFile, uploadSystemFile } from "~/services/FileService";
...
const CreateFile = () => {
  ...
  const handleChange = (e) => {
    ...
    if (
      name === "type" &&
      formData.type !== value &&
      (formData.type === "THEME" || value === "THEME")
    ) {
      setFormData({ ...formData, [name]: value, url: "" });
    } else {
  ...
  return (
    <div className="container mx-auto p-4 max-w-xl bg-white rounded-md shadow-md">
      ...
      <h2 className="text-2xl font-bold mb-4 text-center">Create System File</h2>
      {severity && (
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="relative z-0 w-full mb-5 group">
          ...
          >
            ...
            <option value="SOUNDDONE">SOUNDDONE</option>
            <option value="SOUNDCONCENTRATION">SOUNDCONCENTRATION</option>
          </select>
          <label
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">
            {formData.type === "THEME" ? "Image:" : "Sound:"}
          </label>
          <div className="mt-2 flex flex-col items-center">
            ...
            <input
              type="file"
              accept={formData.type === "THEME" ? "image/*" : "audio/*"}
      ...
      {severity && (
        ...
        >
          {severity === "success" ? "Creation successful!" : "Creation failed."}
        </div>
      )}
    </div>
  );
